...
```{r}
# Define nodes (6 nodes across 4 levels)
...
# Define links (specific strand and others)
links <- data.frame(
  source = c(0, 0, 1, 2, 3, 4),  # Indices of source nodes
  target = c(1, 2, 3, 4, 5, 5),  # Indices of target nodes
  value = c(10, 5, 10, 5, 15, 5), # Link values (widths)
  group = c("highlight", "default", "highlight", "default", "highlight", "default") # Group for highlighting
)

# Define the color of each link
links$color <- "gray"  # Default color for all links

# Highlight the specific strand A → B → D → F in red
highlighted_indices <- which(
  (links$source == 0 & links$target == 1) |  # A → B
  (links$source == 1 & links$target == 3) |  # B → D
  (links$source == 0 & links$target == 5)    # D → F
)

# Set the color of the highlighted links to red
links$color[highlighted_indices] <- "#FF0000"
